...
export function buildSign(occasion, name) {
  let greeting = `Happy ${occasion} ${name}!`;
  return greeting;
}
...
export function buildBirthdaySign(age) {
  let greeting = `Happy Birthday! What a ${age >= 50 ? 'mature' : 'young'} fellow you are.`
  return greeting;
}
...
export function graduationFor(name, year) {
  let greeting = `Congratulations ${name}!\nClass of ${year}`;
...
export function costOf(sign, currency) {
  let cost = `Your sign costs ${Number(sign.length * 2 + 20).toFixed(2)} ${currency}.`
